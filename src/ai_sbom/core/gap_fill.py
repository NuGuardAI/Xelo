...
Integration
-----------
Called from ``SbomExtractor._llm_enrich()`` **before** ``verify_uncertain_nodes``
...
from ai_sbom.models import AiBomDocument, Evidence, Node, NodeMetadata
from ai_sbom.models import SourceLocation
...
def _identify_absent_categories(doc: AiBomDocument) -> list[ComponentType]:
...
def _build_existing_node_summary(doc: AiBomDocument) -> str:
