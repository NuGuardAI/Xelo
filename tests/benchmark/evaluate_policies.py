...
def convert_ground_truth_to_aibom(ground_truth: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(ground_truth, dict) and "nodes" in ground_truth and "schema_version" in ground_truth:
        return ground_truth
...
def evaluate_assertion(
    ...
    if assertion_type == "must_exist":
        ...
        if operator == "exists":
            passed_nodes = [n for n in matching_nodes if _get_property_value(n, property_path) is not None]
...
def _node_matches_query(node: Dict, query: Dict) -> bool:
    ...
    node_props = _node_property_lookup(node)
...
def compare_to_ground_truth(
    ...
    gap_f1 = 2 * gap_precision * gap_recall / (gap_precision + gap_recall) if (gap_precision + gap_recall) > 0 else 0.0
...
def run_policy_benchmark(policy_id: str) -> PolicyBenchmarkResult:
