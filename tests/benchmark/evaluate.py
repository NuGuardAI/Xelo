...
def _convert_xelo_ground_truth_to_legacy(repo_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for node in nodes:
        ...
        asset = {
            ...
            "synonyms": extras.get("synonyms", []) if isinstance(extras.get("synonyms"), list) else [],
...
def path_matches_fuzzy(disc_path: str, gt_path: str) -> bool:
    ...
    if disc_filename == gt_filename and disc_filename:
        ...
        unique_filenames = {'main.py', 'app.py', 'agent.py', 'tools.py', 'crew.py', 
                           'agents.py', 'prompts.py', 'config.py', 'server.py'}
        ...
        for evidence_str in ground_truth.evidence:
            ...
            if len(disc_norm) >= 3 and len(ev_norm) >= 3:
                if disc_norm in ev_norm or ev_norm in disc_norm:
                    if len(min(disc_norm, ev_norm, key=len)) / len(max(disc_norm, ev_norm, key=len)) >= 0.5:
                        return True
...
def evaluate_discovery(
    ...
    Returns:
        EvaluationResult with precision, recall, F1, and details
    """
    ...
    asset_types = set(
        [a.asset_type.value for a in gt_assets] +
        [a.asset_type for a in discovered]
    )
    ...
    false_positive_details = [
        discovered[i].model_dump()
        for i in range(len(discovered))
        if i not in matched_disc_indices
    ]
    false_negative_details = [
        gt_assets[i].model_dump()
        for i in range(len(gt_assets))
...
def _convert_aibom_nodes_to_discovered_assets(
    ...
    for node in aibom_nodes:
        node_type_raw = str(getattr(getattr(node, "type", None), "value", getattr(node, "type", "")) or "").strip()
        ...
        description = ""
        for field_name in ("summary", "description", "purpose", "details", "asset_summary", "content_preview"):
...
async def evaluate_repo(
    ...
    mode_str = "aibom-api" if mode_normalized == "api" else ("regex+LLM" if use_llm else "regex-only")
...
async def evaluate_all(
    ...
    mode_str = "aibom-api" if mode.strip().lower() == "api" else ("regex+LLM" if use_llm else "regex-only")
    ...
    for asset_type in all_types:
        type_tp = sum(r.by_type.get(asset_type, TypeMetrics()).true_positives for r in active_results.values())
        type_fp = sum(r.by_type.get(asset_type, TypeMetrics()).false_positives for r in active_results.values())
        type_fn = sum(r.by_type.get(asset_type, TypeMetrics()).false_negatives for r in active_results.values())
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate NuGuard AI asset discovery accuracy"
    )
    parser.add_argument(
        "--repo",
        type=str,
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Evaluate all benchmark repositories"
    )
    parser.add_argument(
        "--list",
        action="store_true",
        help="List available benchmark repositories"
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
    ...
    parser.add_argument(
        "--token", "-t",
        type=str,
        help="GitHub token for API access (or set GITHUB_TOKEN in .env)"
    )
    ...
    if args.mode == "api" and not args.auth_token and (not args.auth_email or not args.auth_password):
    ...
    if args.repo:
        try:
            ...
            mode_str = "(aibom-api)" if args.mode == "api" else ("(regex+LLM)" if args.llm else "(regex-only)")
            print(f"\n{mode_str}")
    ...
    if args.all:
        ...
        if result.by_type_aggregate:
            ...
            for asset_type, metrics in sorted(result.by_type_aggregate.items()):
                print(f"  {asset_type}: P={metrics.precision:.2%} R={metrics.recall:.2%} F1={metrics.f1_score:.2%}")
