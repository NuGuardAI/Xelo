...
def finding_matches(
    ...
    if flexibility in [MatchFlexibility.EXACT, MatchFlexibility.EXACT_CONTROL, MatchFlexibility.SEMANTIC]:
        ...
        if policy_match and severity_match and (evidence_overlap >= 0.5 or remediation_overlap >= 0.5):
...
def covered_control_matches(
    ...
    if flexibility == MatchFlexibility.SEMANTIC:
        ...
        if (
            ground_truth.control_name.lower() in disc_control_name.lower()
            and normalize_text(disc_policy) == normalize_text(ground_truth.policy_name)
        ):
            return True, "WEAK"
...
def calculate_quality_score(result: RiskEvaluationResult) -> float:
    ...
    risk_accuracy = 1.0 if result.risk_score_within_tolerance else max(
        0.0, 1.0 - (result.risk_score_error / 50)
    )
...
async def evaluate_risk_assessment(
    ...
    for gt_idx, gt_control in enumerate(gt_controls):
        for disc_idx, disc_control in enumerate(discovered_covered_controls):
            ...
            if matched:
                ...
                control_matches_list.append(CoveredControlMatchResult(
    ...
    if ground_truth.expected_red_team_attacks:
        ...
        # Check count
        result.red_team_count_sufficient = len(discovered_red_team_attacks) >= expected_attacks.min_count
    ...
    covered_control_ids = {
        (c.get("control_id"), c.get("policy_name"))
        for c in discovered_covered_controls
    }
...
async def evaluate_repo(
    ...
    logger.info(f"  Ground truth: {len(gt.expected_findings)} findings, "
...
async def evaluate_all(
    ...
    for repo_name in repos:
        try:
            ...
            failed_count += 1
            results.append(RiskEvaluationResult(
                repo_name=repo_name,
                policies_evaluated=[],
                error=str(e),
            ))
    ...
    if successful_results:
        aggregate_finding_f1 = sum(r.finding_metrics.f1_score for r in successful_results) / len(successful_results)
        aggregate_covered_f1 = sum(r.covered_metrics.f1_score for r in successful_results) / len(successful_results)
        aggregate_risk_score_mae = sum(r.risk_score_error for r in successful_results) / len(successful_results)
        aggregate_band_accuracy = sum(1 for r in successful_results if r.band_match) / len(successful_results)
        aggregate_quality_score = sum(r.quality_score for r in successful_results) / len(successful_results)
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate NuGuard AI risk assessment accuracy"
    )
    parser.add_argument(
        "--repo",
        type=str,
        help="Evaluate a specific benchmark repository"
    )
    parser.add_argument(
        "--all",
        action="store_true",
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="List available risk benchmark repositories"
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
    ...
    parser.add_argument(
        "--no-cache",
        action="store_true",
        help="Don't use cached files, always fetch from GitHub"
    )
    parser.add_argument(
        "--policies",
        type=str,
    ...
    try:
        if args.all:
            suite_result = asyncio.run(evaluate_all(
            ...
            if suite_result.aggregate_quality_score >= args.threshold:
                logger.info(f"✓ Quality score {suite_result.aggregate_quality_score:.2f} >= threshold {args.threshold}")
                return 0
            else:
                logger.error(f"✗ Quality score {suite_result.aggregate_quality_score:.2f} < threshold {args.threshold}")
                return 1
        
        else:
            result = asyncio.run(evaluate_repo(
                repo_name=args.repo,
                verbose=args.verbose,
                skip_discovery=args.skip_discovery,
                use_cache=not args.no_cache,
            ...
            if result.quality_score >= args.threshold:
                logger.info(f"✓ Quality score {result.quality_score:.2f} >= threshold {args.threshold}")
                return 0
            else:
                logger.error(f"✗ Quality score {result.quality_score:.2f} < threshold {args.threshold}")
                return 1
