...
def doc() -> AiBomDocument:
    return SbomExtractor().extract_from_path(MILO, _MILO_CONFIG)
...
class TestLLMProviders:
    def test_groq_provider_detected(self, doc: AiBomDocument) -> None:
    ...
    def test_ollama_model_detected(self, doc: AiBomDocument) -> None:
    ...
    def test_gemini_model_detected(self, doc: AiBomDocument) -> None:
        model_names = {n.name.lower() for n in nodes(doc, ComponentType.MODEL)}
        assert any("gemini" in m for m in model_names), (
            f"Expected a gemini model, got: {model_names}"
        )

    def test_yaml_config_adapter_fires(self, doc: AiBomDocument) -> None:
...
class TestDatastore:
    def test_sqlite_detected(self, doc: AiBomDocument) -> None:
...
class TestNginx:
    def test_deployment_from_proxy_pass(self, doc: AiBomDocument) -> None:
    ...
    def test_tls_auth_from_nginx(self, doc: AiBomDocument) -> None:
    ...
    def test_nginx_adapter_used(self, doc: AiBomDocument) -> None:
        adapters_used = {n.metadata.extras.get("adapter") for n in doc.nodes}
...
class TestDockerfile:
    def test_container_image_detected(self, doc: AiBomDocument) -> None:
    ...
    def test_expose_port_detected(self, doc: AiBomDocument) -> None:
    ...
    def test_playwright_tool_from_dockerfile(self, doc: AiBomDocument) -> None:
...
class TestPromptFiles:
    def test_system_prompt_detected(self, doc: AiBomDocument) -> None:
        prompt_nodes = nodes(doc, ComponentType.PROMPT)
        assert prompt_nodes, "Expected at least one PROMPT node from prompts/*.txt"

    def test_prompt_is_template(self, doc: AiBomDocument) -> None:
    ...
    def test_prompt_adapter_used(self, doc: AiBomDocument) -> None:
        adapters_used = {n.metadata.extras.get("adapter") for n in doc.nodes}
